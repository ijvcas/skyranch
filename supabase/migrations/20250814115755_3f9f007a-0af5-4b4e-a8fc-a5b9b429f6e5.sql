-- Phase 1: Enhanced Database Logic for Grazing Time Management

-- Create function to calculate expected exit date based on occupancy
...
BEGIN
  ...
  occupancy_factor := CASE 
    WHEN current_animals::numeric / lot_capacity::numeric > 0.8 THEN 0.7  -- Reduce days by 30% for high occupancy
    WHEN current_animals::numeric / lot_capacity::numeric > 0.6 THEN 0.8  -- Reduce days by 20% for medium-high occupancy
    WHEN current_animals::numeric / lot_capacity::numeric > 0.4 THEN 0.9  -- Reduce days by 10% for medium occupancy
    ELSE 1.0  -- Full days for low occupancy
  END;
...
-- Create function to calculate lot next available date
...
-- Create function to get lot grazing metrics
...
-- Create function to update lot status automatically
...
-- Create trigger for automatic lot status updates
DROP TRIGGER IF EXISTS trigger_update_lot_status ON animal_lot_assignments;
CREATE TRIGGER trigger_update_lot_status
  AFTER INSERT OR UPDATE OF removed_date OR DELETE ON animal_lot_assignments
  FOR EACH ROW
  EXECUTE FUNCTION update_lot_status_on_assignment_change();