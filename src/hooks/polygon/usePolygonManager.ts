...
export const usePolygonManager = (
  ...
  const handlePolygonComplete = useCallback(async (
    ...
    try {
      console.log(`🎯 Saving polygon for existing lot:`, lotId);
      ...
      if (success) {
        ...
        toast.success(`Polígono guardado exitosamente`);
      } else {
        polygon.setMap(null);
      ...
      lots.forEach((lot) => {
        ...
        try {
          ...
          if (!isAutoGenerated && onPolygonClick) {
            polygon.addListener('click', () => {
              console.log(`🖱️ Polygon clicked for user-created lot: ${lot.name}`);
          ...
          if (!isAutoGenerated) {
            polygon.addListener('mouseover', () => {
              polygon.setOptions({ strokeWeight: 3, fillOpacity: polygonOptions.fillOpacity + 0.1 });
            });
            polygon.addListener('mouseout', () => {
              polygon.setOptions({ strokeWeight: polygonOptions.strokeWeight, fillOpacity: polygonOptions.fillOpacity });
            });
          }
