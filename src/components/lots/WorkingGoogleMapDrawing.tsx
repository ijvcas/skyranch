...
const WorkingGoogleMapDrawing = ({ lots, onLotSelect }: WorkingGoogleMapDrawingProps) => {
  ...
  useEffect(() => {
    ...
    lots.forEach(lot => {
      const isPropertyLot = (lot as any).autoGenerated === true;
      const isPastureLot = (lot as any).autoGenerated !== true;
      
      // Check if this lot type should be shown
      // Business rule: On the Potreros map, property (auto-generated) lots must NOT display their Lote number
      const shouldShow = (isPastureLot && showPastureLots);
  ...
  useEffect(() => {
    if (isMapReady && polygons.size > 0) {
      ...
      polygons.forEach((polygon, lotId) => {
        ...
        if (lot) {
          console.log(`📍 Polygon for lot ${lot.name} (${(lot as any).autoGenerated ? 'auto-generated' : 'user-created'}): visible=${polygon.getVisible()}`);
        }
      });
    }
