...
const WorkingGoogleMapDrawing = ({ lots, onLotSelect }: WorkingGoogleMapDrawingProps) => {
  ...
  useEffect(() => {
    console.log('🔍 WorkingGoogleMapDrawing: Total lots received:', lots.length);
    ...
    console.log('🏢 Auto-generated (property) lots:', autoGeneratedLots.length, autoGeneratedLots.map(l => l.name));
    console.log('🌿 User-created (pasture) lots:', userCreatedLots.length, userCreatedLots.map(l => l.name));
    
    // Special debug for P-1
    const p1Lot = lots.find(l => l.name === 'P-1');
    if (p1Lot) {
      console.log('🔍 P-1 lot details:', {
        id: p1Lot.id,
        name: p1Lot.name,
        autoGenerated: p1Lot.autoGenerated,
        currentAnimals: p1Lot.currentAnimals,
        status: p1Lot.status
      });
    } else {
      console.error('❌ P-1 lot not found in lots array!');
    }
  }, [lots]);
  ...
  const handleClearPastureLots = () => {
    console.log('🧹 Clearing pasture lots...');
    const pastureLots = lots.filter(lot => lot.autoGenerated !== true);
    console.log('🌿 Pasture lots to clear:', pastureLots.length);
    pastureLots.forEach(lot => {
      const polygon = polygons.get(lot.id);
      if (polygon) {
        console.log('🗑️ Removing polygon for pasture lot:', lot.name);
        polygon.setMap(null);
      }
    });
  };
  ...
  const handleFitBounds = () => {
    if (!isMapReady || !mapInstance || polygons.size === 0) {
      console.log('⚠️ Cannot fit bounds: map not ready or no polygons');
      return;
    }
    
    console.log('🗺️ Fitting bounds to', polygons.size, 'polygons...');
    forcePolygonVisibility(); // This will also fit bounds and force visibility
  };

  // Debug polygon rendering
  useEffect(() => {
    if (isMapReady && polygons.size > 0) {
      console.log('🗺️ Current polygons on map:', polygons.size);
      polygons.forEach((polygon, lotId) => {
        const lot = lots.find(l => l.id === lotId);
        if (lot) {
          console.log(`📍 Polygon for lot ${lot.name} (${lot.autoGenerated ? 'auto-generated' : 'user-created'}): visible=${polygon.getVisible()}, map=${!!polygon.getMap()}`);
          
          // Special check for P-1
          if (lot.name === 'P-1') {
            console.log('🔍 P-1 POLYGON DEBUG:', {
              lotId,
            ...
            if (!polygon.getVisible()) {
              console.log('🔧 P-1 polygon not visible, forcing visibility...');
      ...
      // Check if P-1 polygon exists but isn't in our polygons map
      const p1LotId = '61cb0814-319f-43e8-9436-08c589899f55';
      if (!polygons.has(p1LotId)) {
        console.error('❌ P-1 polygon missing from polygons map!');
      }
    }
